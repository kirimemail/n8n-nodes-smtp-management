...
export class KirimEmailSmtpWebhookTrigger implements INodeType {
	description: INodeTypeDescription = {
		...
		group: ['trigger'],
		version: 1,
		description: 'Handle Kirim.Email SMTP webhook events',
		defaults: {
			name: 'KirimEmail SMTP Trigger',
		},
		webhooks: [
			{
				name: 'default',
				httpMethod: 'POST',
				path: 'kirim-email-smtp',
	...
	async webhook(this: IWebhookFunctions): Promise<IWebhookResponseData> {
		const credentials = await this.getCredentials('kirimEmailSmtpWebhookApi');
